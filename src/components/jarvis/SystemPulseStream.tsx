...
export function SystemPulseStream({ index = 0 }: { index?: number }) {
  ...
  return (
    <HudPanel index={index} title="SYSTEM PULSE // EVENT STREAM" className="flex flex-col">
      <div className="relative h-40 overflow-hidden p-2 @max-[420px]:h-24">
        {events.length === 0 ? (
          ...
          <ul className="space-y-1">
            {events.map((it, i) => (
              <li
                key={it.id}
                className="flex items-center gap-2 border-l-2 pl-2 font-display text-[10px] uppercase tracking-[0.18em] @max-[420px]:text-[8px]"
                style={{
        ...
        <div
          className="pointer-events-none absolute inset-x-0 bottom-0 h-10 @max-[420px]:h-6"
