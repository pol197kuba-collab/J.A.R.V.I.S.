-- Dev Wing follow-up #2: JARVIS delegated "add a new project + tasks" to
-- D.R.O.I.D. via delegate_to_agent, but delegation only returns the
-- teammate's prose reply — not the raw project id — so JARVIS's own
-- create_task calls afterward had no real project_id to pass and the new
-- tasks landed with project_id NULL (silently dropped into "General").
-- Simplest fix: give JARVIS list_projects/create_project directly so it
-- never needs to round-trip a project's id through another agent's text
-- reply. D.R.O.I.D. keeps them too — this is additive, not a move.

INSERT INTO public.agent_tools (agent_id, tool_id, is_enabled)
SELECT a.id, t.id, true
FROM public.agents a
CROSS JOIN public.tools t
WHERE a.slug = 'jarvis' AND t.slug IN ('list_projects', 'create_project')
