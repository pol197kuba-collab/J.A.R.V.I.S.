-- Dev Wing follow-up #3: JARVIS created a project + task for real dev work
-- ("stwórz grę 2D...") but had no start_dev_session/check_dev_session tools
-- of its own, and its prompt never told it to delegate the actual dispatch
-- step to D.R.O.I.D. either — it just created the task, then told the user
-- "zleciłem D.R.O.I.D.-owi zadanie" even though nothing was ever sent to
-- GitHub. Grants JARVIS the two dispatch tools directly (same reasoning as
-- the earlier list_projects/create_project grant: avoid round-tripping
-- through another agent's prose) and the orchestrator prompt now tells it
-- to actually call start_dev_session in the same turn it creates a dev task.

INSERT INTO public.agent_tools (agent_id, tool_id, is_enabled)
SELECT a.id, t.id, true
FROM public.agents a
CROSS JOIN public.tools t
WHERE a.slug = 'jarvis' AND t.slug IN ('start_dev_session', 'check_dev_session')
...
BEGIN
  ...
  IF v_agent_id IS NOT NULL THEN
    ...
    WHERE t.slug IN (
      ...
      'run_local_action', 'queue_document_job', 'list_projects', 'create_project',
      'start_dev_session', 'check_dev_session'
    )
    ON CONFLICT (agent_id, tool_id) DO NOTHING;
  END IF;
