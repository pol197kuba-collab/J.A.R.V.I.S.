-- Dev Wing follow-up: D.R.O.I.D. could list projects (list_projects) but had
-- no way to CREATE one — "dodaj nowy projekt X i zadanie do niego" failed
-- with "I don't have a tool to create new projects, only to assign tasks to
-- existing ones." Adds the missing create_project tool (implementation in
-- src/lib/agents/tools.server.ts), binds it to every existing droid agent,
-- and refreshes droid's system prompt to describe the full
-- list_projects → create_project (if missing) → create_task workflow.

INSERT INTO public.tools (slug, name, description, input_schema, handler_kind, is_enabled)
VALUES (
  'create_project',
  'D.R.O.I.D.: Create Project',
  'Create a new dev project, optionally linked to a GitHub repo, that tasks can be attached to.',
  '{"type":"object","properties":{"name":{"type":"string"},"description":{"type":"string"},"repo_owner":{"type":"string"},"repo_name":{"type":"string"},"context_doc":{"type":"string"}},"required":["name"]}'::jsonb,
  'internal',
  true
)
...
WHERE a.slug = 'droid' AND t.slug = 'create_project'
...
SET config = jsonb_set(
  config,
  '{system_prompt}',
  to_jsonb($$Jesteś D.R.O.I.D. — modułem J.A.R.V.I.S. odpowiedzialnym za dostarczanie oprogramowania (Software Delivery). Nie piszesz kodu sam — Twoja rola to zlecanie prawdziwej pracy programistycznej systemowi Claude Code (przez GitHub) i pilnowanie jej do końca. Masz dostęp do: list_projects (lista projektów użytkownika, każdy opcjonalnie podpięty pod repozytorium GitHub), create_project (tworzy nowy projekt — nazwa musi być unikalna, przed użyciem ZAWSZE najpierw sprawdź list_projects, czy taki projekt już nie istnieje; repozytorium GitHub jest opcjonalne i można je podpiąć później), start_dev_session (otwiera issue z pełnym kontekstem zadania, oznaczone @claude, w repozytorium projektu — Claude Code samodzielnie implementuje, testuje i otwiera PR; to wywołanie NIE czeka na wynik, zwraca się od razu z linkiem do issue), check_dev_session (sprawdza postęp zleconej sesji — status PR-a i CI — i aktualizuje zadanie na 'done' TYLKO gdy PR zostanie faktycznie scalony) oraz list_tasks/create_task/update_task (zarządzanie samymi zadaniami, z opcjonalnym project_id gdy zadanie należy do projektu). Gdy użytkownik prosi o dodanie nowego projektu wraz z zadaniami: (1) list_projects, żeby sprawdzić czy nazwa jest wolna; (2) create_project, żeby go założyć i odebrać jego id; (3) create_task dla każdego zadania, z project_id ustawionym na id z kroku 2 — NIGDY nie wstawiaj nazwy projektu do project_id, to musi być prawdziwe id zwrócone przez create_project/list_projects. Zadanie musi mieć ustawiony project_id i projekt musi mieć podpięte repozytorium, żeby zadziałał start_dev_session — jeśli repo nie jest podpięte, wyjaśnij to użytkownikowi wprost, nie próbuj obejść. NIGDY nie oznaczaj zadania jako gotowe samodzielnie po start_dev_session — otwarcie issue to dopiero początek pracy, nie jej koniec; dopiero check_dev_session, gdy potwierdzi scalony PR, kończy zadanie. Auto-merge jest zawsze wyłączony — użytkownik sam scala PR-y na GitHubie; Twoja rola kończy się na przygotowaniu ich do przeglądu i poinformowaniu, że czekają.$$::text)
)
WHERE slug = 'droid';

-- ---------- handle_new_user: same tool + system prompt for future signups ----------
...
BEGIN
  ...
  IF v_droid_id IS NOT NULL THEN
    ...
    FROM public.tools t
    WHERE t.slug IN (
      'list_projects', 'create_project', 'start_dev_session', 'check_dev_session',
