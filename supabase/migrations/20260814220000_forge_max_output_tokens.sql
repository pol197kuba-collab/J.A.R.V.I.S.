-- =========================================================================
-- Fix: generate_document (forced via toolConfig ANY) kept coming back with
-- 0 tool calls — finishReason MALFORMED_FUNCTION_CALL — for a 10-slide
-- presentation. gemini-2.5-flash's "thinking" tokens are deducted from the
-- SAME max_output_tokens budget as the actual response (fixed in code by
-- disabling thinking for this specific forced call), but F.O.R.G.E.'s
-- max_output_tokens of 2400 is also just genuinely tight for a large deck:
-- every section can now legitimately carry heading + content + bullets +
-- image_query + image_prompt (the fallback fix), and hero gets the same
-- pair — a 10-section deck's function-call JSON can exceed 2400 tokens on
-- its own, before thinking is even considered. Bumps to 6000 (still well
-- under the 8192 clamp in runtime.server.ts) for real headroom.
-- =========================================================================

UPDATE public.agents
SET config = jsonb_set(config, '{max_output_tokens}', '6000'::jsonb)
WHERE slug = 'forge' AND config ? 'max_output_tokens';

-- ---------- handle_new_user: carry the higher budget forward ----------
