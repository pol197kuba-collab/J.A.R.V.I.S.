-- =========================================================================
-- Producer stage 2 — real web photos (Openverse) preferred over AI graphics.
--
-- The AI image model (gemini-2.5-flash-image) is chronically 503-throttled,
-- so decks kept coming out with no graphics. Stage 2 adds real Creative-
-- Commons photos via image_query / hero_image_query (producer.server.ts
-- fetchWebImage → Openverse), preferred over the AI-prompt fields. This
-- migration teaches the persona to prefer photo queries and refreshes the
-- tool's informational input_schema with the new fields.
--
...
UPDATE public.tools
SET input_schema = '{"type":"object","properties":{"format":{"type":"string","enum":["pptx","docx","pdf"]},"title":{"type":"string"},"subtitle":{"type":"string"},"filename":{"type":"string"},"hero_image_query":{"type":"string"},"hero_image_prompt":{"type":"string"},"sections":{"type":"array","items":{"type":"object","properties":{"heading":{"type":"string"},"content":{"type":"string"},"bullets":{"type":"array","items":{"type":"string"}},"image_query":{"type":"string"},"image_prompt":{"type":"string"}},"required":["heading"]}}},"required":["format","title","sections"]}'::jsonb
