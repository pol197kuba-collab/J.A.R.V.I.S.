-- =========================================================================
-- Producer prompt fix — stop the model from hand-retyping download links.
--
-- Live failure (first flagship-demo test): the persona instructed the
-- model to paste download_url VERBATIM into its reply. An LLM retyping a
-- ~300-char signed-URL token reliably introduces typos — the generated
-- pptx link failed with InvalidJWT "signature verification failed" (the
-- pdf's luckier copy worked). The runtime now appends captured links to
-- the reply by code (runtime.server.ts `attachments`), so personas must
-- do the opposite: never retype URLs.
--
--   1. Updates config.system_prompt of every existing producer agent.
--   2. Redeclares handle_new_user() with the same corrected prompt.
-- =========================================================================
